...
mean_estim_phyrex <- read.csv(file = file.path(here_dir, "results", paste0(datestamp_day, "_mean_estim_phyrex.csv")))
mean_estim_beast <- read.csv(file = file.path(here_dir, "results", paste0(datestamp_day, "_mean_estim_beast.csv")))
...
ggsave(filename = file.path(here_dir, "results", paste0("velocity_ess_", sample_every, ".pdf")),
       plot = p,
       width = columnwidth,
       height = columnwidth,
       unit = "in")
